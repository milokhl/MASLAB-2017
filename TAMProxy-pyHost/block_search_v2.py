# -*- coding: utf-8 -*-
"""
Created on Mon Jan 30 23:31:14 2017
...
class BlockSearchV2(SyncedSketch):
    #motor pins
    ...
    colorSenseMaxgr = S.colorSenseMaxgr
    ourColor = S.ourColor
    
    minFraction = 0.005 #percent of pixels needed to be considered a block
    endGame = False

    #overallState: -1 = Do nothing, 0 = drive to next coordinate, 1=drive to block, 2=collect block, 3=get path to next block (threaded),4 = blocks dropped, drive backwards
    overallState = 0
    ...
    #load path from file
    #TODO
    pathCoordinates = [(80, 111), (56, 111), (80, 87), (85, 124, 57.52880770915151)]
    targetCoord = pathCoordinates[0]   
    #load occupancy grids from file
    #TODO
    x = 0.0
    y = 0.0
    init_heading = 0
    theta_est = S.theta_est - math.degrees(init_heading)
    ...
    def loop(self):
        if (self.roundTimer.millis()<=175e3) and shouldContinue: #If there's still enough time to collect blocks
            ...
            if self.timer.millis() > self.refresh_rate:
                ...
                if self.calibrated:
                    ...
                    """
                    if self.roundTimer.millis()>150e3:
                        self.lowOnTime = True                    
                    ...
                    #Image Processing
                    #Switches to state 1 if there is a block sighted and to state 0 if no block sighted
                    if self.shouldProcessImage():
                        self.processImage(ret)
                        
                    #Perform action based on state
                    if self.overallState==0:
                        #Drive to next coordinate
                        self.drive_to_coord(*self.targetCoord)
                        
                    elif self.overallState == 1:
                        #center on block and drive to it
                        self.driveToBlock()
                        ...
                        #get the path to original path
                        #Do Nothing and wait for path to calculate
                        self.bias = 0
                    ...
                    elif self.overallState == 4:
                        #drop blocks and drive backward
                        self.relinquishStack()
                    ...
                    #Dead reckon and update motors
                    self.updateThetaEstNoEncoder()
                    self.calculatePower()
                    self.updateMotors()
                    self.deadReckon()
                                    
                    #When we have collected enough blocks or are low on time. Skip to heading to the base
                    if (self.numOurBlocks == 5 or self.lowOnTime) and (not self.headingToBase):
                        #TODO set a goal and put in settings
                        #start planning path back to HQ
                        self.headingToBase = True
                        self.targetCoord = self.pathCoordinates[-1]
                        self.pathCoordinates = [self.pathCoordinates[-1]]
                        self.overallState = 3
                        
                    
                    #If close enough to next point on path according to dead reckoning, move to next point
                    if (np.hypot(self.x-self.targetCoord[0],self.y-self.targetCoord[1])<=2  and self.last_diff<=5 and (self.overallState not in [-1,4])):
                        print "Moving to next coordinate"
                        self.pathCoordinates.pop()
                        self.driveToCoordState = 0
                        if len(self.pathCoordinates)==1:
                            self.headingToBase = True
                        if len(self.pathCoordinates)>0:
                            self.targetCoord = self.pathCoordinates[0]
                        else:
                            self.overallState=4
            ...
            if self.statusTimer.millis() > 100:
                ...
                print "ourBlockPUS:",self.ourBlockPUS
                print "oppBlockPUS:",self.oppBlockPUS
                print "relinquishStackState:",self.relinquishStackState
                print "dropState:",self.dropState
                print "driveBackState",self.driveBackState
                
                print "lowOnTime:",self.lowOnTime
                print "headingToBase",self.headingToBase
...
#            pass
    ...
    #-------------------------------------------------------------------------
    #PID and Dead Reckoning
            
    def updateThetaEstNoEncoder(self):
        #Only use gyroscope to estimate current angle
        self.theta_est = self.gyro.val  - math.degrees(self.init_heading)

    def updateThetaEstEncoder(self):

        self.theta_est_enc = float(self.left_motor_encoder.val - self.right_motor_encoder.val)*self.wheel_radius*9.0/(80.0*self.axel_length) - math.degrees(self.init_heading)
...
#        self.derivative = (diff - self.last_diff)/self.dt
    ...
    def shouldProcessImage(self):
#        return ((self.overallState ==0) or (self.drivingToBlockState<1 and self.overallState==1)) and (not (self.lowOnTime or self.headingToBase))
        return False

    def processImage(self,ret):
        allCnt = []
        allCntNoButtons = []
        if ret==True:
...
#        return sortedCnts
    ...
    #-------------------------------------------------------------------------           
    #State 0 Drive to Coord. 
    def drive_to_coord(self, x, y, theta = None):
    ...
    #-------------------------------------------------------------------------
    #State 1 drive to block
    def driveToBlock(self):
        toleranceAng = 10
        changingDistance = 15.0
        extraDistance = 5.0        
#        print self.angle
...
#                self.blockPUS = self.checkColor()
    ...
    def checkColor(self):
        ...
        if blockColor=="None":
            ...
            return 1
    #-------------------------------------------------------------------------
    #State 3 find way back to path (potentially by threading outside of sketch)
    #-------------------------------------------------------------------------
    #State 4 drop stack and drive back                    

    def relinquishStack(self):
        if self.relinquishStackState==0:
            #drop our stack and open door
            self.dropStack()
        elif self.relinquishStackState==1:
            #back up
            self.driveBack()                
...
if __name__ == "__main__":
    ...
    sketch = BlockSearchV2(1, -0.00001, 100)
