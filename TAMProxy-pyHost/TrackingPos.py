# -*- coding: utf-8 -*-
"""
Created on Sat Jan 14 21:19:44 2017
...
class DriveSquare(SyncedSketch):
    ...
    last_diff = 0.0
    integral = 0.0
    axel_length = S.axel_length #inches
    wheel_radius = S.wheel_radius #inches
...
#                distance = (self.left_motor_encoder.val + self.right_motor_encoder.val) / 6400.0 * self.wheel_circumference
        ...
        if self.timer2.millis() > 100:
            ...
            print "pos:",(self.x,self.y,-numpy.degrees(self.heading))
            print "Dist:",self.distTravelled
...
#        self.theta_est = self.gyro.val
        self.theta_est = -numpy.degrees(self.heading)
    ...
    def deadReckon(self):
        ...
        if (abs(deltaL - deltaR)) <= 1e-6:
            dx = deltaL * numpy.cos(self.heading)
            dy = deltaR * numpy.sin(self.heading)

#            self.heading = numpy.radians(-self.gyro.val)

        else:
            r = self.axel_length * (deltaL + deltaR) / (2 * (deltaR - deltaL))
            wd = (deltaR - deltaL) / self.axel_length
            
            dx = r* numpy.sin(self.heading + wd) - r*numpy.sin(self.heading)
            dy = -r* numpy.cos(self.heading + wd) + r*numpy.cos(self.heading)
            self.heading += wd
