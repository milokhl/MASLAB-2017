...
class DriveToCoord(SyncedSketch):
    ...
    driveToCoordState = 0
#    coordinates = [[84, 0], [132, 72],[132, -60], [96,24], [0,0,0]]
#    coordinates = [[48, 0, None], [0, 0,0]]
    coordinates = [(-24, 0), (0, -24), (5, 13, 57.5)]
    coord_ind = 0
    ...
    def loop(self):
        if self.timer.millis() > self.refresh_rate:
            ...
            if self.calibrated:
                try:
                   # print "here"
                    self.drive_to_coord(*self.coordinates[self.coord_ind])
                except IndexError:
...
#                self.heading += wd
    ...
    def drive_to_coord(self, x, y, theta = None):
        if self.driveToCoordState == 0:
            self.desired_theta = self.getClosestEquivalentAngle(self.get_coord_angle(x, y))
            ...
            if theta == None:
                ...
                angle = self.getClosestEquivalentAngle(-theta)
