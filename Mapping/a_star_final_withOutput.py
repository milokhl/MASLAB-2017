...
def isClearPath(SEG, allSegments, tolerance = 1.0):
...
def get_clean_path(start, goal,boundaryGrid,bounds, maxY):
        cNodes = find_path(start, goal, boundaryGrid, maxY)
        path = find_path_1(cNodes)
        path_clean = cleanPath(path, bounds)
        return path_clean
...
##    goal = stacks[3] #bug at stack[3], stack[4]
    ...
    # set coordinates that we want to drive to here
    goals = [start, stacks[0], stacks[1], homePos]
    ...
    for i in range(len(goals) - 1):
        path_clean = get_clean_path(goals[i], goals[i +1], boundaryGrid,bounds, maxY)
    ...
    t3 = time.time()
    path_total[len(path_total)-1] = (homePos[0], homePos[1], final_heading) # change the final path position into a coordinate plus heading
    
    np.savez(mapFileName + "output", init_heading=init_heading, path_total = np.array(path_total), bounds = np.array(bounds), boundaryGrid = np.array(boundaryGrid), maxX = maxX, maxY = maxY)
